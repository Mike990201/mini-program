...
Page({
    ...
    async HandleChooseAddress(e){
        //调用获取收货地址的api
        var address = await chooseAddress();
    ...
    async HandleItemNumEdit(e){
        ...
        let {cart}=this.data;
        let Index= cart.findIndex((v,i)=>{
            return goods_id===v.goods_id
        })
        if(cart[Index].num===1 && operation ===-1){
            const res=await showModel({content:"是否要删除商品？"});
            if (res.confirm) {
                cart.splice(Index,1);
                this.setCart(cart);
            }
